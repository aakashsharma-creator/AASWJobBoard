...
<?php 
	...
	if ( empty( $packages ) ) {
		echo '<div class="jobpostpage notice notice-warning" style="padding:10px; background:#fff3cd; border:1px solid #ffeeba;">
				You have no package or your consumed all your job listings. Please select a new package from the subscription menu to continue posting jobs.
			  </div>';
	} else {
		// Check if user has packages but all are consumed
		...
		if ( $result > 0 ) {
			echo '<div class="jobpostpage notice notice-warning" style="padding:10px; background:#fff3cd; border:1px solid #ffeeba;">
					You have consumed all your job listings. Please select a new package from the subscription menu to continue posting jobs.
				  </div>';
		}
	}
?>
	...
	<?php if ( job_manager_user_can_post_job() || job_manager_user_can_edit_job( $job_id ) ) : ?>
		...
		//if ( !empty( $packages ) ) { ?> 
			<p>
				...
				<?php if ('v3' === $captcha_version) echo 'onclick="jm_job_submit_click(event)"'; ?> value="Preview & Submit<?php //echo esc_attr($submit_button_text); ?>" />
				<?php
				if ( isset( $can_continue_later ) && $can_continue_later ) {
					echo '<input type="submit" name="save_draft" class="button secondary save_draft" value="' . esc_attr__( 'Save as Draft', 'wp-job-manager' ) . '" formnovalidate />';
				}
				?>
