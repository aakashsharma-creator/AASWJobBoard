...
$dashboardUrl = site_url()."/job-management/"; 
...
switch ( $job->post_status ) :
	case 'publish' :
		$job_submitted_content = '<div class="job-manager-message">' . wp_kses_post(
			sprintf(
				...
				__( '%1$s listed successfully. <a href="'.$dashboardUrl.'">Go to Job Dashboard</a>', 'wp-job-manager' ),
	...
	case 'pending' :
		...
		if ( $job_dashboard_link && empty( $job_dashboard_title ) ) {
			...
			$job_submitted_content .= wp_kses_post(
				sprintf(
					__( '  <a href="%s" class=""> %s</a>', 'wp-job-manager' ),
					$job_dashboard_link,
					$job_dashboard_title
				)
			);
