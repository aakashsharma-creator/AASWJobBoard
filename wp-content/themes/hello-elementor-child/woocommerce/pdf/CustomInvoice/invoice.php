...
<?php if ( $this->has_header_logo() ) : ?>
	<h1 class="document-type-label"><?php //$this->title(); ?> Tax Invoice/Receipt</h1>
<?php endif; ?> 
	...
	<thead>
		<tr>
			<?php foreach ( wpo_wcpdf_get_simple_template_default_table_headers( $this ) as $column_class => $column_title ) : ?>
				...
				?>
				<th style="background-color: #AA243F; <?php echo $alignright; ?> " class="<?php echo esc_attr( $column_class ); ?>">
					...
					if($column_title == 'Product'){
						if (stripos($itemName, 'room') !== false) {
							echo "Room Package";
						} else {
							echo "Job Package";
						}
	...
	<tbody>
		<?php foreach ( $this->get_order_items() as $item_id => $item ) : ?>
			<tr class="<?php echo esc_html( $item['row_class'] ); ?>">
				...
				<td class="price" style="text-align: right;"><?php echo esc_html( $item['order_price'] ); ?></td>
			</tr>
		<?php endforeach; ?>
	</tbody>
</table>
			...
			<td class="no-borders totals-cell">
				<table class="totals">
					<tfoot>
						...
						<?php 
						// custom code
						// Reorder totals: Subtotal → GST → Payment Processing Fee → Total
						$subtotal_row = $gst_row = $fee_row = $total_row = null;
						$other_rows = [];

						foreach ( $this->get_woocommerce_totals() as $total ) {
							$label = strtolower( $total['label'] );

							if ( strpos( $label, 'subtotal' ) !== false ) {
								$subtotal_row = $total;
							} elseif ( strpos( $label, 'gst' ) !== false ) {
								$gst_row = $total;
							} elseif ( strpos( $label, 'payment processing fee' ) !== false ) {
								$fee_row = $total;
							} elseif ( strpos( $label, 'total' ) !== false ) {
								$total_row = $total;
							} elseif ( strpos( $label, 'payment method' ) !== false ) {
								// Skip payment method row (do not show)
								continue;
							} else {
								$other_rows[] = $total;
							}
						...
						// Output in custom order
						if ( $subtotal_row ) : ?>
							<tr><th><?php echo $subtotal_row['label']; ?></th><td style="text-align: right;"><?php echo $subtotal_row['value']; ?></td></tr>
						<?php endif; ?>

						<?php if ( $gst_row ) : ?>
							<tr><th><?php echo $gst_row['label']; ?></th><td style="text-align: right;"><?php echo $gst_row['value']; ?></td></tr>
						<?php endif; ?>

						<?php if ( $fee_row ) : ?>
							<tr><th><?php echo $fee_row['label']; ?></th><td style="text-align: right;"><?php echo $fee_row['value']; ?></td></tr>
						<?php endif; ?>

						<?php foreach ( $other_rows as $total ) : ?>
							<tr><th><?php echo $total['label']; ?></th><td style="text-align: right;"><?php echo $total['value']; ?></td></tr>
						<?php endforeach; ?>

						<?php if ( $total_row ) : ?>
							<tr class="order-total"><th><?php echo $total_row['label']; ?></th><td style="text-align: right;"><?php echo $total_row['value']; ?></td></tr>
						<?php endif; ?>
